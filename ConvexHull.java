...
public class ConvexHull {
     ...
     * @return return 1 for counterclockwise
     *                -1 for clockwise
     *                0 for collinear
     */
    ...
    private static void sortConvexHullVertices(List<int[]> convexHullVertices){
        ...
        for(int i = 0; i < convexHullVerticesSize; i++){
            ...
            convexHullVertices.get(i)[0] *= convexHullVerticesSize;
            convexHullVertices.get(i)[1] *= convexHullVerticesSize;
        }
    ...
    private static int[][] randomPoints(int size, int range){
        ...
        for(int i = 0; i < size; i++){
            points[i][0] = random.nextInt(range);
            points[i][1] = random.nextInt(range);
        }
        return points;
...
//        int[][] points;
        ...
        for(int size = 10; size <= 30000010; size += 1000000){
        ...
        // + " (theoretical running time: " + (size * Math.log(size) / Math.log(2)) + "ns)" + "ratio = " + ((endTime - startTime) / (size * Math.log(size) / Math.log(2)))
    }
}
...
class counterclockwiseComparator implements Comparator<int[]>{
    ...
    int[] centroid;
    public counterclockwiseComparator(int[] centroid){
        this.centroid = centroid;
    }
    private int quadrant(int[] point){
        // the first quadrant
    ...
    public int compare(int[] point0, int[] point1){
        int[] centroidRelative0 = {point0[0] - centroid[0], point0[1] - centroid[1]};
        int[] centroidRelative1 = {point1[0] - centroid[0], point1[1] - centroid[1]};
        ...
        return point1[0] * point0[1] - point0[0] * point1[1];
    }
}